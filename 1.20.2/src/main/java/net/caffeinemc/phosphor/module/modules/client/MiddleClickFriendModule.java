    ...
    private void onHudRender(HudRenderEvent event) {
        if (render.isEnabled()) {
            ...
            renderFriends.forEach((String plusOrMinus, String playerName) -> {
                event.context.drawText(mc.textRenderer, String.format("%s %s", plusOrMinus, playerName), 5, offsetY.get(), plusOrMinus.equals("+") ? Color.GREEN.getRGB() : Color.RED.getRGB(), true);
